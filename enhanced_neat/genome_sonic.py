...
class DefaultGenome(object):
    ...
    def mutate(self, config):
        ...
        prob_add_node = config.node_add_prob*self.mutation_rate
        if random() < prob_add_node:
            self.mutate_add_node(config)

        if random() < config.node_delete_prob*self.mutation_rate:
            self.mutate_delete_node(config)
        ...
        total_fitness_change = sum([gene.fitness_change for gene in self.connections.values()])

        for gene_key, gene in self.connections.items():
            
            if abs(total_fitness_change) > 0:
                # Adjust the mutation rate based on the fitness change.
                if gene.fitness_change >= 0:
                    # If the fitness improved, increase the mutation rate.
                    gene.add_connection_prob *= self.mutation_rate*(1 + abs(gene.fitness_change/total_fitness_change))
                else:
                    # If the fitness did not improve, decrease the mutation rate.
                    gene.remove_connection_prob *= self.mutation_rate*(1 - abs(gene.fitness_change/total_fitness_change))
                    gene.remove_node_prob *= self.mutation_rate*(1 - abs(gene.fitness_change/total_fitness_change))

        total_fitness_change = sum([gene.fitness_change for gene in self.nodes.values()])

        for gene_key, gene in self.nodes.items():
    ...
    def mutate_add_connection(self, config):
        ...
        for k, v in list(self.nodes.items()):
            if random() < v.add_connection_prob:
    ...
    def mutate_delete_node(self, config):
        ...
        # available_nodes = []
        # for k, v in iteritems(self.nodes):
        #     if random() < v.remove_node_prob:
    ...
    def mutate_delete_connection(self):
        if self.connections:
            for k, v in list(self.connections.items()):
                if random() < v.remove_connection_prob:
